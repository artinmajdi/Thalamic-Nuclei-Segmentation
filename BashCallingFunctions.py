...
def RigidRegistration(Input , Template):
    ...
    BiasCorrected = Input.Address + '/' + Input.Files.BiasCorrected + '.nii.gz' 
    Output = Input.Address + '/Crop.nii.gz' 
    ...
    if not os.path.isfile( Output ):
        ...
        os.system("WarpImageMultiTransform 3 %s %s -R %s linearAffine.txt"%(Template.Mask , Output , BiasCorrected) )


def BiasCorrection(Dir,name):

    if not os.path.isfile( Dir + '/' + name + '_bias_corr.nii.gz' ):
