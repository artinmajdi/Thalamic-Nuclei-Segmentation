...
#     processed = subject.Address + '/' + subject.origImage + '_bias_corr.nii.gz'
    processed = subject.Address + '/' + subject.Files.ImageProcessed + '.nii.gz'
    outP = subject.Files.Temp.Address + '/CropMask.nii.gz'
...
def BiasCorrection(subject , preprocess):
    ...
    inP  = subject.Address + '/' + subject.Files.ImageProcessed + '.nii.gz'
    outP = subject.Address + '/' + subject.Files.ImageProcessed + '.nii.gz'
    ...
    if preprocess.Mode and preprocess.BiasCorrection.Mode:
        ...
        if preprocess.Debug.Mode:
            copyfile(outP , subject.Files.Temp.Address + '/' + subject.Files.ImageOriginal + '_bias_corr.nii.gz')
...
def Bash_Cropping(subject , preprocess):
    ...
    if preprocess.Mode and preprocess.Cropping.Mode:
        ...
        inP  = subject.Address + '/' + subject.Files.ImageProcessed + '.nii.gz'
        outP = subject.Address + '/' + subject.Files.ImageProcessed + '.nii.gz'
        crop = subject.Files.Temp.Address + '/CropMask.nii.gz'
        os.system("ExtractRegionFromImageByMask 3 %s %s %s 1 0"%( inP , outP , crop ) )

        if preprocess.Debug.Mode:
            copyfile(outP , subject.Files.Temp.Address + '/' + subject.Files.ImageOriginal + '_bias_corr_Cropped.nii.gz')

        # Cropping the Label
        inP  = subject.Files.Label.Address + '/' + subject.Files.Label.LabelProcessed + '.nii.gz'
        outP = subject.Files.Label.Address + '/' + subject.Files.Label.LabelProcessed + '.nii.gz'
        crop = subject.Files.Temp.Address + '/CropMask.nii.gz'
        os.system("ExtractRegionFromImageByMask 3 %s %s %s 1 0"%( inP , outP , crop ) )

        if preprocess.Debug.mode:           
            copyfile(outP , subject.Files.Label.Temp.Address + '/' + subject.Files.Label.LabelOriginal + '_Cropped.nii.gz')        

def Bash_AugmentNonLinear(subject , subjectRef , outputAddress): # Image , Mask , Reference , output):

    ImageOrig = subject.Address       + '/' + subject.Files.ImageProcessed + '.nii.gz' 
    MaskOrig  = subject.Files.Label.Address + '/' + subject.Files.Label.LabelProcessed + '.nii.gz'   
    ImageRef  = subjectRef.Address    + '/' + subjectRef.Files.ImageProcessed + '.nii.gz' 

    OutputImage = outputAddress  + '/' + subject.Files.ImageProcessed + '.nii.gz' 
    labelAdd = mkDir(outputAddress + '/Label') 
    OutputMask  = labelAdd + '/' + subject.Files.Label.LabelProcessed + '.nii.gz' 
